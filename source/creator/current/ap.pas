...
if not fileexists(ffname) then
  ...
  try
    mainfm.sb1.Panels[0].text:='Pulling Down Project';
    result:=true;
...
if not fileexists(ts) then
  ...
  while not EOF(tf) do
    ...
    case ord(tc) of
      ...
      79: begin
          { O - Number of compiles }
          compileNo:=strtoint(ts);
          end;
      37: begin
      ...
      81: begin
          { Q - flag to exclude file }
          if ts='1' then
            curfile.exclude:=true
          else
            curfile.exclude:=false;
          end;
      58: begin
          { : }
          curfile.reg.add(ts);
...
writeln(tfile,'O'+inttostr(compileno));
...
begin
mainfm.sb1.Panels[0].text:='Making Test';
result:=false;
{ set values to 0 }
...
var
  ...
  tempra:trecapp;
begin
mainfm.sb1.Panels[0].text:='Making Backup';
result:=true;
...
var
  tempra:trecapp;
begin
mainfm.sb1.Panels[0].text:='Making Redeployment';
result:=true;
{ set values to 0 }
...
var
  ...
  parm:string;
begin
mainfm.sb1.Panels[0].text:=status;
...
res:= messagedlg('Delete all Project Definition Files for "'+nname+'" ? This command cannot be undone.',mtconfirmation,[mbyes,mbno],0);
if res=mrYes then
  begin
  fn:=ffname;
...
begin
mainfm.sb1.Panels[0].text:='Refreshing Redeployment';
result:=true;
{ get latest md5's }
version:=newver;
...
begin
mainfm.sb1.Panels[0].text:='ReDeploying Project';
result:=true;
...
begin
result:=true;
mainfm.sb1.Panels[0].text:='Receiving Project';
...
begin
result:=true;
mainfm.sb1.Panels[0].text:='Clearing Staging Area';
{ need to calculate tp }
res:=messagedlg('Clear out Staging Area of "'+nname+'" ?'#13#10#13#10+
