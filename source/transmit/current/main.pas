...
type
  Tmainfm = class(TForm)
    ...
    Exit1: TMenuItem;
    Help1: TMenuItem;
    AboutUpdateTransmitter1: TMenuItem;
    tb1: TToolBar;
    sb1: TStatusBar;
    Settings1: TMenuItem;
    ...
    N4: TMenuItem;
    UpgradeTransmitterHelp1: TMenuItem;
    AppExec1: TAppExec;
    ras: TMMJRasConnect;
    procedure Exit1Click(Sender: TObject);
    procedure Formcreate(Sender: TObject);
    procedure AboutUpdateTransmitter1Click(Sender: TObject);
...
uses  ShellAPI, about, settings, view, fwall,share;
...
begin
goon:=true;
sb1.Panels[0].text:='Deploying Project';
...
sb1.Panels[0].text:='Deployment Finished';
viewfm.m1.lines.adD('');
...
begin
goon:=true;
sb1.Panels[0].text:='Pulling Down Project';
result:=true;
{ get directory from staging area that files will be in }
curapp:=tap.create;
...
if goon then
  createupdatefile('');
sb1.Panels[0].text:='Pull Down Finished';
end;
...
procedure Tmainfm.AboutUpdateTransmitter1Click(Sender: TObject);
begin
aboutbox.showmodal;
end;
...
begin
{ get rid of this at nearest oppurtunity }
end;
...
if paramcount<7 then
  begin
  showmessage('This was not called from the Upgrade Suite Server or Client programs.');
  halt;
  end;
end;
...
if (paramcount<>8) or
   ...
   begin
   showmessagE('Transmitter incorrectly called');
   deletestatusfile;
   halt;
   end;
{ Loads parameters }
...
assignfile(tempf,ts);
rewrite(tempf);
writeln(tempf,'Transmitter started');
closefile(tempf);
except
